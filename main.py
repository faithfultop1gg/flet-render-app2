...
def main(page: ft.Page):
    ...
    header = ft.Container(
        content=ft.Text("Что нового, Артем!", size=24, weight=ft.FontWeight.BOLD, color="#2D3748"),
        padding=ft.padding.only(bottom=30)
    )

    categories_text = ft.Container(
        content=ft.Text("КАТЕГОРИИ", size=16, weight=ft.FontWeight.BOLD, color="#718096"),
    ...
    def show_topic_menu(e, topic):
        ...
        back_button = ft.IconButton(icon=ft.Icons.ARROW_BACK, on_click=lambda e: show_main_menu(e))
        topic_header = ft.Container(
            content=ft.Text(topic, size=20, weight=ft.FontWeight.BOLD, color="#2D3748"),
            padding=ft.padding.only(bottom=30)
        )
        ...
        study_button = ft.Container(
            ...
            padding=20, margin=10, bgcolor="white", border_radius=10, width=150, height=120,
            on_click=lambda e: show_rules(e, topic), ink=True
        )
        ...
        practice_button = ft.Container(
            ...
            padding=20, margin=10, bgcolor="white", border_radius=10, width=150, height=120,
            on_click=lambda e: start_practice(e, topic), ink=True
        )

        page.add(ft.Row([study_button, practice_button], alignment=ft.MainAxisAlignment.CENTER))

    def show_rules(e, topic):
        page.clean()

        back_button = ft.IconButton(icon=ft.Icons.ARROW_BACK, on_click=lambda e: show_topic_menu(e, topic))
        rules_header = ft.Container(
            content=ft.Text(f"Правила: {topic}", size=20, weight=ft.FontWeight.BOLD, color="#2D3748"),
            padding=ft.padding.only(bottom=20)
        )

        rules_text = ft.Container(
            content=ft.Text(topics_data[topic]["rules"], size=16, color="#4A5568"),
            padding=20, bgcolor="white", border_radius=10, margin=ft.margin.only(bottom=20)
        )
        ...
        for word_data in topics_data[topic]["words"]:
            examples_column.controls.append(
                ft.Container(
                    content=ft.Text(word_data['word'].replace('_', word_data['correct']),
                                    size=14, color="#4A5568"),
                    padding=10, bgcolor="#F7FAFC", border_radius=5, margin=ft.margin.only(bottom=5)
                )
            )

        start_practice_btn = ft.ElevatedButton(
            "Начать тренировку", on_click=lambda e: start_practice(e, topic),
            style=ft.ButtonStyle(color="white", bgcolor="#4C6EF5"), width=200
        )

        page.add(ft.Row([back_button]), rules_header, rules_text, examples_header,
                 examples_column, ft.Container(content=start_practice_btn,
    ...
    def show_practice_word():
        ...
        progress = ft.Container(
            content=ft.Text(f"{current_word_index + 1}/{len(topics_data[topic]['words'])}",
                            size=14, color="#718096"),
            alignment=ft.alignment.center
        )

        word_display = ft.Container(
            content=ft.Text(word_data["word"], size=32, weight=ft.FontWeight.BOLD, color="#2D3748"),
            padding=30, margin=20, alignment=ft.alignment.center,
        ...
        for option in word_data["options"]:
            option_btn = ft.Container(
                content=ft.Text(option, size=24, weight=ft.FontWeight.BOLD, color="white"),
                width=80, height=80, alignment=ft.alignment.center,
                bgcolor="#4C6EF5", border_radius=40,
                on_click=lambda e, opt=option: check_answer(e, opt), ink=True
            )
    ...
    def show_results():
        ...
        result_text = ft.Container(
            content=ft.Column([
                ...
                ft.Text("Тренировка завершена!", size=24, weight=ft.FontWeight.BOLD, color="#2D3748"),
                ft.Text(f"Результат: {score}/{total_words}", size=20, color="#4A5568")
            ], alignment=ft.MainAxisAlignment.CENTER, horizontal_alignment=ft.CrossAxisAlignment.CENTER),
            padding=40, alignment=ft.alignment.center
        )

        restart_button = ft.ElevatedButton("Повторить тренировку",
                                           on_click=lambda e: start_practice(e, topic),
                                           style=ft.ButtonStyle(color="white", bgcolor="#4C6EF5"),
                                           width=200)
        back_button = ft.TextButton("Вернуться к темам", on_click=show_main_menu)
    ...
    def create_topic_button(topic_name):
        return ft.Container(
            ...
            on_click=lambda e: show_topic_menu(e, topic_name), ink=True
        )
